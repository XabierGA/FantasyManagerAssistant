...
class DataStorage:
    ...
    def save_to_csv(self, news_items: List[NewsArticle], filename: Optional[str] = None) -> str:
        ...
        if filename is None:
            filename = f"futbol_fantasy_news_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        
        filepath = os.path.join(self.data_dir, filename)
        
        # Convert data classes to dictionaries
        ...
        if os.path.exists(filepath):
            try:
                ...
                combined_df = pd.concat([existing_df, df]).drop_duplicates(subset=['title', 'link'])
                combined_df.to_csv(filepath, index=False)
                logger.info(f"Updated existing file {filepath} with {len(df)} new items")
    ...
    def save_to_json(self, news_items: List[NewsArticle], filename: Optional[str] = None) -> str:
        ...
        if filename is None:
            filename = f"futbol_fantasy_news_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
