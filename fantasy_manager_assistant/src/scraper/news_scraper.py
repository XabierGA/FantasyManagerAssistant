...
class WebScraper:
    ...
    def normalize_url(self, url: str) -> str:
        ...
        return self.base_url.rstrip('/') + ('/' if not url.startswith('/') else '') + url
...
class FutbolFantasyScraper(WebScraper):
    ...
    def parse_news_items(self, html_content: str) -> List[NewsArticle]:
        ...
        # Find all news containers based on the class we identified
        news_containers = soup.select('div.noticia-container')
        logger.info(f"Found {len(news_containers)} news containers")
        
        for container in news_containers:
            try:
                # Find the news link which contains the title and points to the full article
                news_link = container.select_one('a.noticia')
                
                if not news_link:
                    continue
                
                # Extract title (alt attribute of the link)
                title = news_link.get('alt', '').strip()
                if not title:
                    # If alt is empty, try to get text from the link
                    title = news_link.text.strip()
                
                # Extract link URL and normalize it
    ...
    def fetch_article_content(self, article: NewsArticle) -> str:
        ...
        # Look for the main article content using common content container selectors
        content_selectors = [
            'div.article-content', 
            'div.entry-content', 
            'div.post-content',
        ...
        if paragraphs:
            # Filter out short paragraphs that might be navigation/headers
            substantial_paragraphs = [p.text.strip() for p in paragraphs if len(p.text.strip()) > 60]
...
class ScraperRunner:
    ...
    def run_single(self, fetch_content: bool = False, save_format: str = 'csv') -> List[NewsArticle]:
    ...
    def run_scheduled(self, interval_minutes: int = 60, fetch_content: bool = False, save_format: str = 'csv'):
        ...
        Args:
            ...
            save_format: The format to save the data in ('csv' or 'json').
        """
        logger.info(f"Starting scheduled scraper with {interval_minutes} minute interval")
        
        while True:
            try:
                self.run_single(fetch_content=fetch_content, save_format=save_format)
                logger.info(f"Sleeping for {interval_minutes} minutes before next scrape")
